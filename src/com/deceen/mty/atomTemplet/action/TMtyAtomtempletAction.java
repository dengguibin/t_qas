package com.deceen.mty.atomTemplet.action;
...
import com.deceen.mty.atomTemplet.service.TMtyAtomtempletService;
 ...
 * @date2017-11-10 12:04:39
 * @version V1.0
 */
@Scope("prototype")
@Component("tMtyAtomtempletAction")
@SuppressWarnings({ "rawtypes", "serial", "unused" })
public class TMtyAtomtempletAction extends BaseActionSupport {

	private static final Logger log = Logger.getLogger(TMtyAtomtempletAction.class);

	private TMtyAtomtempletService service;

	private TMtyAtomtemplet bean;
	...
	public String save() throws Exception {
		if (bean != null) {
			...
			if (bean.getId() != null && "".equals(bean.getId()) == false) {
				...
				TMtyAtomtemplet dsBean = service.select(bean.getId());
				if (dsBean != null) {
					// 如果数据库中有此记录信息，进行更新操作
	...
	public TMtyAtomtempletService getService() {
		return service;
	}

	@Autowired
	public void setService(TMtyAtomtempletService service) {
		this.service = service;
	}

	public TMtyAtomtemplet getBean() {
		return bean;
	}

	public void setBean(TMtyAtomtemplet bean) {
		this.bean = bean;
	}
