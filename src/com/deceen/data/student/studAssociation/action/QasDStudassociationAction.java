package	com.deceen.data.student.studAssociation.action;
...
* @date2017-11-24 10:29:01
* @version V1.0   
*/
@Scope("prototype")
@Component("qasDStudassociationAction")	
@SuppressWarnings({ "rawtypes", "unchecked", "serial" ,"unused" })
public class QasDStudassociationAction  extends BaseActionSupport{

	private static final Logger log = Logger.getLogger(QasDStudassociationAction.class);

	private  QasDStudassociationService service;
	
	private QasDStudassociation bean;
	...
	public String save() throws Exception {
		if(bean != null  ){
			...
			if(bean.getId() != null && "".equals(bean.getId()) == false){
				...
				QasDStudassociation dsBean = service.select(bean.getId());
				if(dsBean != null){
				// 如果数据库中有此记录信息，进行更新操作
					service.update(bean);
				}else{
...
//			}	
	...
	public QasDStudassociationService getService() {
		return service;
	}

	@Autowired
	public void setService(QasDStudassociationService service) {
		this.service = service;
	}

	public QasDStudassociation getBean() {
		return bean;
	}

	public void setBean(QasDStudassociation bean) {
		this.bean = bean;
	}
