...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/fund/fundIncomeStatistics/";
...
<div class="dataGrid">	
<form action="${ctx}/data/fund/fundIncomeStatistics/queryList.action"   method="post" 
 		...
 		<table>
            <tbody >
             <tr>
                ...
                <th>学校总收入：</th>
				<td>
					<select class="select2" name="bean.schoolincomeScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.schoolincomeScope}">
								<option value='${bean.schoolincomeScope}'  selected="selected">${bean.schoolincomeScope}</option>
							</c:if>
							<c:if test="${bean.schoolincomeScope!='0-5000'}">
								 <option value="0-5000">0-5000</option>
							</c:if>
							<c:if test="${bean.schoolincomeScope!='5000-8000'}">
								 <option value="5000-8000">5000-8000</option>
							</c:if>
							<c:if test="${bean.schoolincomeScope!='8000-10000'}">
								<option value="8000-10000">8000-10000</option>
							</c:if>
							<c:if test="${bean.schoolincomeScope!='10000-15000'}">
								 <option value="10000-15000">10000-15000</option>
							</c:if>
							<c:if test="${bean.schoolincomeScope!='15000以上'}">
                 ...
                 <th>创建人：</th>
                <td><s:textfield name="bean.createdby" cssClass="inputText"></s:textfield></td>
                <td>
                
                <th>创建时间：</th>
                <td>
                	<s:textfield name="bean.createdtime" cssClass="inputDate" onClick="new WdatePicker()"></s:textfield>
                </td>
                
                 <th>修改人：</th>
                <td><s:textfield name="bean.updatedby" cssClass="inputText"></s:textfield></td>
                <td>
                </tr>
                <tr>
                <th>修改时间：</th>
		  ...
		  <table id="dataTable" class="list_table">
            <thead>
              <tr>
               ...
               <!--  <th>序号</th> -->
                <th>学校总收入（万元）</th>
                <th>其中<-->学费收入<-->合计（万元）</th>
                <th>其中<-->财政经常性补助收入<-->合计（万元）</th>
                <th>其中<-->中央、地方财政专项投入<-->合计（万元）</th>
                <th>其中<-->社会捐赠金额（万元）</th>
                <!-- <th>其中其他收入总额（万元）</th> -->
               <!--  <th>贷款金额（万元）</th>
                <th>贷款余额（万元）</th> -->
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	           <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<%-- <td>&nbsp;${item.ID}</td> --%>
                   	<td>&nbsp;${item.SCHOOLINCOME}</td>
                   	<td>&nbsp;${item.TUITIONTOTAL}</td>
                   	<td>&nbsp;${item.FINANCIALAIDTOTAL}</td>
                   	<td>&nbsp;${item.SPECIALINVESTMENTTOTAL}</td>
                   	<td>&nbsp;${item.SOCIALDONATION}</td>
                   	<%-- <td>&nbsp;${item.OTHERS}</td> --%>
