...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/major/graduateEmploymentUnits/";
...
<div class="dataGrid">	
<form action="${ctx}/data/major/graduateEmploymentUnits/queryList.action"   method="post" 
		...
		<div class="filter">
 		<table>
            <tbody >
             <tr>
                <th>应届毕业生主要就业单位名称：</th>
                <td><s:textfield name="bean.mainemploymentunit" cssClass="inputText"></s:textfield></td>
                <th>选择是否做过雇主调查：</th>
                <td>
                 <select  id="select9" name="bean.isemployersurvey" style="width:160px;">
                 	<option value="">--请选择--</option>
                 	<!--调用字典的效果-->
				   	 <x:cache subject="10010" option="true" selected="${ben.isemployersurvey }"></x:cache>
				   </select>
                </td>
                <%-- <td>
                <th>选择菜单：</th>
		  ...
		  <table id="dataTable" class="list_table">
            <thead>
              <tr>
                ...
                <th width="50">序号</th>
                <!-- <th>记录标识</th> -->
                <th>应届毕业生主要就业单位名称(全称)</th>
                <th>是否做过雇主调查</th>
                <th>就业单位联系人信息<-->姓名</th>
                <th>就业单位联系人信息<-->职务</th>
                <th>就业单位联系人信息<-->区号－电话号码</th>
                <th>就业单位联系人信息<-->电子邮箱</th>
               <!--  <th>状态</th>
                <th>创建人</th>
                <th>创建时间</th>
                <th>修改人</th> -->
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr>
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<%-- <td>&nbsp;${item.ID}</td> --%>
                   	<td>&nbsp;${item.MAINEMPLOYMENTUNIT}</td>
                   		<c:if test="${item.ISEMPLOYERSURVEY=='1'||item.ISEMPLOYERSURVEY=='是'}">
							<td>&nbsp;是</td>
						</c:if>
						<c:if test="${item.ISEMPLOYERSURVEY=='2'||item.ISEMPLOYERSURVEY=='否'}">
							<td>&nbsp;否</td>
						</c:if>
						<c:if test="${empty item.ISEMPLOYERSURVEY}">
							<td>&nbsp;${item.ISEMPLOYERSURVEY}</td>
						</c:if>
                   	<td>&nbsp;${item.CONTACTNAME}</td>
                   	<td>&nbsp;${item.CONTACTDUTY}</td>
