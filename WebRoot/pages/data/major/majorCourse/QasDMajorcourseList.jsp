...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/major/majorCourse/";
...
<div class="dataGrid">	
<form action="${ctx}/data/major/majorCourse/queryList.action"   method="post" 
		...
		<div class="filter">
 		<table>
            <tbody >
             <tr>
                <th>课程名称：</th>
                <td><s:textfield name="bean.courseid" cssClass="inputText"></s:textfield></td>
                <td>
                <!-- <th>选择菜单：</th>
                <td>
                ...
                <th>选择主要授课地点：</th>
                <td>
                 <select  id="select9" name="bean.classaddress" style="width:160px;">
                 	<option value="">--请选择--</option>
                 	<!--调用字典的效果-->
				   	 <x:cache subject="10052" option="true" selected="${bean.classaddress }"></x:cache>
				   </select>
                </td>
		  ...
		  <table id="dataTable" class="list_table">
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr>
					...
					<c:if test="${item.CLASSADDRESS=='普通教室'}">
						<td>&nbsp;普通教室</td>
					</c:if>   
					<c:if test="${item.CLASSADDRESS=='一体化教室'}">
						<td>&nbsp;一体化教室</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='机房'}">
						<td>&nbsp;机房</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='多媒体教室'}">
						<td>&nbsp;多媒体教室</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='实践场所'}">
						<td>&nbsp;实践场所</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='校外实训基地'}">
						<td>&nbsp;校外实训基地</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='其他(语音室、体育馆等)'}">
						<td>&nbsp;其他(语音室、体育馆等)</td>
					</c:if> 
					<c:if test="${item.CLASSADDRESS=='两个及以上场所'}">
						<td>&nbsp;两个及以上场所</td>
					</c:if> 
					<c:if test="${empty item.CLASSADDRESS}">
						<td>&nbsp;${item.CLASSADDRESS}</td>
					</c:if>                 	
                   	<td>&nbsp;${item.CLASSTYPE}</td>
                   	<td>&nbsp;${item.EXAMEMETHOD}</td>
                 <%--   	<td>&nbsp;${item.PAPERTOCOURSE}</td>
                   	<td>&nbsp;${item.STATUS}</td>
