...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/major/graduateInformation/";
...
<div class="dataGrid">	
<form action="${ctx}/data/major/graduateInformation/queryList.action"   method="post" 
		...
		<div class="filter">
 		<table>
            <tbody >
             <tr>
                <th>学生名称：</th>
                <td><s:textfield name="bean.studentid" cssClass="inputText"></s:textfield></td>
                <td>
                <th>就业省份：</th>
                <td><s:textfield name="bean.employmentprovinces" cssClass="inputText"></s:textfield></td>
                <td>
                <!-- <th>选择菜单：</th>
                <td>
                ...
                <th>选择是否获得全部学分：</th>
                <td>
                 <select  id="select9" name="bean.isreceiveallcredits" style="width:160px;">
                 	<option value="">--请选择--</option>
                 	<!--调用字典的效果-->
				   	 <x:cache subject="10010" option="true" selected="${bean.isreceiveallcredits }"></x:cache>
				   </select>
                </td>
              <!--   
                <th>时间选择：</th>
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr>
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	...
                   	<td>&nbsp;${item.EMPLOYMENTPROVINCES}</td>
                   	<td>&nbsp;${item.COMPANYNAME}</td>
                   	<td>&nbsp;${item.COMPANYNATURE}</td>
                   	<td>&nbsp;${item.COMPANYSCALE}</td>
                   	<c:if test="${item.ISRECEIVEALLCREDITS=='1'}">
                   		<td>&nbsp;是</td>
                   	</c:if>
                   	<c:if test="${item.ISRECEIVEALLCREDITS=='2'}">
                   		<td>&nbsp;否</td>
                   	</c:if>
                   	<c:if test="${empty item.ISRECEIVEALLCREDITS}">
                   		<td>&nbsp;${item.ISRECEIVEALLCREDITS}</td>
                   	</c:if>
                   	<%-- <td>&nbsp;${item.STATUS}</td> --%>
