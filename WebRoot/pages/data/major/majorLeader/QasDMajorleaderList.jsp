...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/major/majorLeader/";
...
<div class="dataGrid">	
<form action="${ctx}/data/major/majorLeader/queryList.action"   method="post" 
            ...
            <tbody >
             <tr>
               ...
               <!--  <th>选择菜单：</th>
                ...
                <%-- <th>选择是否专业带头人：</th>
                <td>
                 <select  id="select9" name="bean.ismajorleader" style="width:160px;">
                 	<option value="">--请选择--</option>
                 	<!--调用字典的效果-->
				   	 <x:cache subject="10010" option="true" selected="${bean.ismajorleader }"></x:cache>
				   </select>
                </td> --%>
                
                 <th>选择是否专业负责人：</th>
                <td>
                 <select id="select9" name="bean.ismajorhead" style="width:160px;">
                 	<option value="">--请选择--</option>
                 	<!--调用字典的效果-->
				   	 <x:cache subject="10010" option="true" selected="${bean.ismajorhead }"></x:cache>
				   </select>
                </td>
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr>
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	...
                   	<%-- <td>&nbsp;${item.HEADYEAR}</td>
                   	<td>&nbsp;${item.LEADERYEAR}</td>
                   	<td>&nbsp;${item.PROJECTNAME}</td>
                   	<td>&nbsp;${item.PROJECTSUMMARY}</td>
                   	<td>&nbsp;${item.PROJECTAWARD}</td>
                   	<td>&nbsp;${item.AWARDDATE}</td>
                   	<td>&nbsp;${item.COOPRATION}</td> --%>
                   	<td>&nbsp;${item.ISMAJORLEADING}</td>
	                   	<c:if test="${item.ISMAJORLEADER=='1'}">
	                   		<td>&nbsp;是</td>
	                   	</c:if>
	                   	<c:if test="${item.ISMAJORLEADER=='2'}">
	                   		<td>&nbsp;否</td>
	                   	</c:if>
	                   	<c:if test="${empty item.ISMAJORLEADER}">
	                   		<td>&nbsp;${item.ISMAJORLEADER}</td>
	                   	</c:if>
                   	
	                   	<c:if test="${item.ISMAJORHEAD=='1'}">
	                   		<td>&nbsp;是</td>
	                   	</c:if>
	                   	<c:if test="${item.ISMAJORHEAD=='2'}">
	                   		<td>&nbsp;否</td>
	                   	</c:if>
	                   	<c:if test="${empty item.ISMAJORHEAD}">
	                   		<td>&nbsp;${item.ISMAJORHEAD}</td>
	                   	</c:if>
                   	<%-- <td>&nbsp;${item.STATUS}</td> --%>
