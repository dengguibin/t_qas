...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/school/booksInLibrary/";
...
<div class="dataGrid">	
<form action="${ctx}/data/school/booksInLibrary/queryList.action"   method="post" 
 		...
 		<table>
            <tbody >
             <tr><!--
                ...
                <th>纸质图书（万册）总册数：</th>
				<td>
					<select class="select2" name="bean.numofbookScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.numofbookScope}">
								<option value='${bean.numofbookScope}'  selected="selected">${bean.numofbookScope}</option>
							</c:if>
							<c:if test="${bean.numofbookScope!='0-50'}">
								<option value="0-50">0-50</option>
							</c:if>
							<c:if test="${bean.numofbookScope!='50-100'}">
								<option value="50-100">50-100</option>
							</c:if>
							<c:if test="${bean.numofbookScope!='100-200'}">
								<option value="100-200">100-200</option>
							</c:if>
							<c:if test="${bean.numofbookScope!='200-300'}">
								<option value="200-300">200-300</option>
							</c:if>
							<c:if test="${bean.numofbookScope!='300以上'}">
                ...
                <th>入库时间：</th>
                <td>
                	<s:textfield name="bean.storagetime" cssClass="inputDate" onClick="new WdatePicker()"></s:textfield></td>
                </td>
                <td>
		  ...
		  <table id="dataTable" class="list_table">
            <thead>
              <tr>
                ...
                <!-- <th>序号</th> -->
                <th>纸质图书（万册）<-->总册数</th>
                <th>纸质图书（万册）<-->本学年新增数</th>
                <th>专业期刊（种）<-->中文纸质专业期刊</th>
                <th>专业期刊（种）<-->外文纸质专业期刊</th>
                <th>专业期刊（种）<-->电子专业期刊</th>
                <th>状态</th>
               <!--  <th>创建人</th>
                <th>创建时间</th>
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	             <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<%-- <td>&nbsp;${item.ID}</td> --%>
                   	<td>&nbsp;${item.NUMOFBOOK}</td>
                   	<td>&nbsp;${item.NUMOFNEWBOOK}</td>
                   	<td>&nbsp;${item.CHINESEJOURNAL}</td>
                   	<td>&nbsp;${item.FOREIGNJOURNAL}</td>
                   	<td>&nbsp;${item.ELECTRONICJOURNAL}</td>
                   	<td>&nbsp;${item.STATUS}</td>
                   	...
                   	<td>&nbsp;${item.TENANTID}</td> --%>
                   	<td>&nbsp;<fmt:formatDate value="${item.STORAGETIME}" pattern="yyyy-MM-dd"/></td>
