...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/school/socialDonation/";
...
<div class="dataGrid">	
<form action="${ctx}/data/school/socialDonation/queryList.action"   method="post" 
		...
		<div class="filter" style="width: 100%;">
 		...
 		<table>
            <tbody >
             <tr>
                <th>捐赠单位：</th>
                <td><s:textfield name="bean.donationdepartment" cssClass="inputText"></s:textfield></td>
                
                 <th>捐赠项目：</th>
                <td><s:textfield name="bean.donationproject" cssClass="inputText"></s:textfield></td>
                
                 <th>捐赠日期（年月）：</th>
                <td><s:textfield name="bean.donationdate" cssClass="inputText"></s:textfield></td>
                
                 <th>捐赠性质：</th>
                <td><s:textfield name="bean.donationnature" cssClass="inputText"></s:textfield></td>
                </tr>
                
                <tr>
                <th>捐赠价值（万元）：</th>
				<td>
					<select class="select2" name="bean.donationvalueScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.donationvalueScope}">
								<option value='${bean.donationvalueScope}'  selected="selected">${bean.donationvalueScope}</option>
							</c:if>
							<c:if test="${bean.donationvalueScope!='0-50'}">
								<option value="0-50">0-50</option>
							</c:if>
							<c:if test="${bean.donationvalueScope!='50-100'}">
								<option value="50-100">50-100</option>
							</c:if>
							<c:if test="${bean.donationvalueScope!='100-200'}">
								<option value="100-200">100-200</option>
							</c:if>
							<c:if test="${bean.donationvalueScope!='200-300'}">
								<option value="200-300">200-300</option>
							</c:if>
							<c:if test="${bean.donationvalueScope!='300以上'}">
		  ...
		  <table id="dataTable" class="list_table">
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	             <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<%-- <td>&nbsp;${item.ID}</td> --%>
                   	<td>&nbsp;${item.DONATIONDEPARTMENT}</td>
                   	<td>&nbsp;${item.DONATIONPROJECT}</td>
                   	<td>&nbsp;${item.DONATIONDATE}</td>
                   	<td>&nbsp;${item.DONATIONNATURE}</td>
                   	<td>&nbsp;${item.DONATIONVALUE}</td>
                   	<td>&nbsp;${item.STATUS}</td>
