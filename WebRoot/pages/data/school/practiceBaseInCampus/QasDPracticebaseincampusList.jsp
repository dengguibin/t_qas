...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/school/practiceBaseInCampus/";
...
<div class="dataGrid">	
<form action="${ctx}/data/school/practiceBaseInCampus/queryList.action"   method="post" 
            ...
            <tbody >
             <tr>
                ...
                <td><s:textfield name="bean.name" cssClass="inputText"></s:textfield></td>
                 ...
                 <th>立项部门名称：</th>
                <td><s:textfield name="bean.supportdepartment" cssClass="inputText"></s:textfield></td>
                
                 <th>实训基地项目批准日期：</th>
                <td><s:textfield name="bean.approvaldate" cssClass="inputText"></s:textfield></td>
                </tr>
                <tr>
                
                <th>建筑面积（平方米）：</th>
				<td>
					<select class="select2" name="bean.constructionareaScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.constructionareaScope}">
								<option value='${bean.constructionareaScope}'  selected="selected">${bean.constructionareaScope}</option>
							</c:if>
							<c:if test="${bean.constructionareaScope!='0-50'}">
								<option value="0-50">0-50</option>
							</c:if>
							<c:if test="${bean.constructionareaScope!='50-100'}">
								<option value="50-100">50-100</option>
							</c:if>
							<c:if test="${bean.constructionareaScope!='100-200'}">
								<option value="100-200">100-200</option>
							</c:if>
							<c:if test="${bean.constructionareaScope!='200-300'}">
								<option value="200-300">200-300</option>
							</c:if>
							<c:if test="${bean.constructionareaScope!='300以上'}">
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	             <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<!--<td>&nbsp;${item.ID}</td>-->
                   	<td>&nbsp;${item.NAME}</td>
                   	<td>&nbsp;${item.SUPPORTDEPARTMENT}</td>
                   	<td>&nbsp;${item.APPROVALDATE}</td>
                   	<td>&nbsp;${item.CONSTRUCTIONAREA}</td>
                   	<td>&nbsp;${item.ISCOCONSTRUCTION}</td>
                   	<td>&nbsp;${item.STATUS}</td>
