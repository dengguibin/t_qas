...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/school/skillAppraisalOrgStatistics/";
...
<div class="dataGrid">	
<form action="${ctx}/data/school/skillAppraisalOrgStatistics/queryList.action"   method="post" 
		...
		<div class="filter" style="width: 100%;">
 		...
 		<table>
            <tbody >
             <tr>
                <th>职业技能鉴定站（所）：</th>
                <td><s:textfield name="bean.skillappraisalorgid" cssClass="inputText"></s:textfield></td>
                
                <th>社会鉴定数（人次）：</th>
				<td>
					<select class="select2" name="bean.societyScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.societyScope}">
								<option value='${bean.societyScope}'  selected="selected">${bean.societyScope}</option>
							</c:if>
							<c:if test="${bean.societyScope!='0-500'}">
								<option value="0-500">0-500</option>
							</c:if>
							<c:if test="${bean.societyScope!='500-1000'}">
								<option value="500-1000">500-1000</option>
							</c:if>
							<c:if test="${bean.societyScope!='1000-2000'}">
								<option value="1000-2000">1000-2000</option>
							</c:if>
							<c:if test="${bean.societyScope!='2000-3000'}">
								<option value="2000-3000">2000-3000</option>
							</c:if>
							<c:if test="${bean.societyScope!='3000以上'}">
				...
				<th>在校生鉴定数（人次）：</th>
				<td>
					<select class="select2" name="bean.studentinschoolScope">
							<option value='--请选择--'>--请选择--</option>
							<c:if test="${!empty bean.studentinschoolScope}">
								<option value='${bean.studentinschoolScope}'  selected="selected">${bean.studentinschoolScope}</option>
							</c:if>
							<c:if test="${bean.studentinschoolScope!='0-500'}">
								<option value="0-500">0-500</option>
							</c:if>
							<c:if test="${bean.studentinschoolScope!='500-1000'}">
								<option value="500-1000">500-1000</option>
							</c:if>
							<c:if test="${bean.studentinschoolScope!='1000-2000'}">
								<option value="1000-2000">1000-2000</option>
							</c:if>
							<c:if test="${bean.studentinschoolScope!='2000-3000'}">
								<option value="2000-3000">2000-3000</option>
							</c:if>
							<c:if test="${bean.studentinschoolScope!='3000以上'}">
								<option value="3000以上">3000以上</option>
							</c:if>
					</select>
				</td>
                ...
                <tr>
                <th>工种/证书名称(全称)：</th>
                <td><s:textfield name="bean.certificate" cssClass="inputText"></s:textfield></td>
                
                <th>工种/证书等级：</th>
                <td><s:textfield name="bean.level" cssClass="inputText"></s:textfield></td>
		  ...
		  <table id="dataTable" class="list_table">
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	             <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	<%-- <td>&nbsp;${item.ID}</td> --%>
                   	<td>&nbsp;${item.SKILLAPPRAISALORGID}</td>
                   	<td>&nbsp;${item.SOCIETY}</td>
                   	<td>&nbsp;${item.STUDENTINSCHOOL}</td>
