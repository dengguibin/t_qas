...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/teacher/teacherTraining/";
...
<div class="dataGrid">	
<form action="${ctx}/data/teacher/teacherTraining/queryList.action"   method="post" 
id="opForm" name="opForm">
	...
	<s:hidden name="bean.id" id="id" />
		...
		<div class="tools">
			<ul class="toolbar">
			<c:if test="${qx.add }"><li class="click add"  onclick="opreate('','add');"><%--<span><img src="${ctx }/skins/uimaker/images/t01.png" /></span>--%>添加</li></c:if>
	        <c:if test="${qx.delete }"><li  class="click remove"  onclick="opreate('','remove');"><%--<span><img src="${ctx }/skins/uimaker/images/t03.png" /></span>--%>删除</li></c:if>
	        </ul>
		</div><!--tools end-->
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr>
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	...
                   	<td>&nbsp;${item.TEACHINGHOUR}</td>
                   	<td>&nbsp;${item.NATIONALITY}</td>
                   	<td>&nbsp;${item.TRAININGSITE}</td>
                   	<td>&nbsp;${item.TRAININGDISPATCHEDDEPARTMENT}</td>
                   	<td>&nbsp;${item.STATUS}</td>
