...
<!--
	function opreate(args, opCode) {
		var baseUrl = "${ctx}/data/student/studAssociation/";
...
<body>
	...
	<div class="dataGrid">
		<form action="${ctx}/data/student/studAssociation/queryList.action"
					...
					<tbody>
						<tr>
							...
							<td><input name="button" onclick="opreate('','query');"
								type="button" class="inputButton" value=" 查 询 " />
							</td>
						</tr>
					</tbody>
				</table>
					...
					<c:forEach items="${datas}" var="item" varStatus="stat">
						<tr align="center">
							<td><input name="test" type="checkbox" class="chk"
								value="${item.ID }" checkNode="Selector" /></td>
							<td>&nbsp;${stat.count }</td>
							<%-- <td>&nbsp;${item.ID}</td> --%>
							...
							<td>&nbsp;${item.APPROVALUNIT}</td>
							<td>&nbsp;${item.REGISTRATIONUNIT}</td>
							<td>&nbsp;${item.MEMBERCOUNT}</td>
							<td>&nbsp;${item.HEADNAME}</td>
							<%-- <td>&nbsp;${item.HEADINGRADE}</td>
							<td>&nbsp;${item.HEADDUTY}</td>
							<td>&nbsp;${item.ISCREDIT}</td>
							<td>&nbsp;${item.GUIDANCEDEPARTMENT}</td>
							<td>&nbsp;${item.GUIDANCETEACHER}</td>
