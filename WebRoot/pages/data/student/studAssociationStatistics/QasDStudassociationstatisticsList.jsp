...
<!--
		function  opreate(args,opCode){
			var baseUrl = "${ctx}/data/student/studAssociationStatistics/";
...
<div class="dataGrid">	
<form action="${ctx}/data/student/studAssociationStatistics/queryList.action"   method="post" 
		...
		<div class="tools">
		  ...
		  <table id="dataTable" class="list_table">
            <thead>
              ...
              <!--   <th>活动经费来源（元--会费</th>
                <th>活动经费来源（元）--学校拨款</th>
                <th>活动经费来源（元）--留存自用的捐款</th>
                <th>活动经费来源（元）--其他</th> -->
                <th>捐赠情况总数（元）</th>
                <!-- <th>捐赠情况（元）--上交业务主管单位	</th>
                <th>捐赠情况（元）--留存自用主要活动内容</th> -->
                <th>培训情况总数（人次）</th>
                 <!--<th>培训情况--获得证书数（本）</th>
                <th>造血干细胞（人次）--采集数</th>
                <th>造血干细胞（人次）--配对数	</th> -->
            ...
            <c:forEach items="${datas}" var="item"  varStatus="stat">
	            <tr align="center">
	                ...
	                <td>&nbsp;${stat.count }</td>
                   	...
                   	<td>&nbsp;${item.PARTICIPANTINACTIVITY}</td>
                   	<td>&nbsp;${item.MEMBERTOTAL}</td>
                   	<td>&nbsp;${item.EMPLOYEEMEMBERNUM}</td>
                   	<td>&nbsp;${item.STUDENTMEMBERNUM}</td>
                   	<td>&nbsp;${item.DUENUM}</td>
                   	<td>&nbsp;${item.HANDINNUM}</td>
                   	<td>&nbsp;${item.REMAINNUM}</td>
                   	<td>&nbsp;${item.ACTIVITYFUND}</td>
                   	<td>&nbsp;${item.ACTIVITYDUE}</td>
                   	<td>&nbsp;${item.ACTIVITYFUNDFROMSCHOOL}</td>
                   	<td>&nbsp;${item.ACTIVITYREMAIN}</td>
                   	<td>&nbsp;${item.ACTIVITYOTHERS}</td>
                   	<td>&nbsp;${item.DONATIONTOTAL}</td>
                   	<td>&nbsp;${item.DONATIONHANDIN}</td>
                   	<td>&nbsp;${item.DONATIONREMAIN}</td>
                   	<td>&nbsp;${item.TRAININGTOTAL}</td>
                   	<td>&nbsp;${item.TRAININGCERTCOUNT}</td>
                   	<td>&nbsp;${item.HEMATOPOIETICSTEMCELLCOUNT}</td>
                   	<td>&nbsp;${item.HEMATOPOIETICSTEMCELLPAIR}</td>
                   	<td>&nbsp;${item.STATUS}</td>
