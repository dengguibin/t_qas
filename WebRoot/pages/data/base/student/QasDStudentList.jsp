...
<!--
	function opreate(args, opCode) {
		var baseUrl = "${ctx}/data/base/student/";
...
<body>
	...
	<div class="dataGrid">
		<form action="${ctx}/data/base/student/queryList.action" method="post"
			...
			<div class="filter">
				<table>
					<tbody>
						<tr>
							<th>学生姓名：</th>
							<td><s:textfield name="bean.name" cssClass="inputText"></s:textfield>
							</td>
							<td>
							<th>选择是否毕业：</th>
							<td><select id="select9" name="bean.isgraduation" style="width:160px;">
							...
							<%-- <th>入学时间选择：</th>
							<td>
								<s:textfield name="bean.admissiondate" cssClass="inputText" ></s:textfield>
							</td> --%>
				...
				<tbody class="tableHover">
					<c:forEach items="${datas}" var="item" varStatus="stat">
						<tr>
							...
							<%-- <td>&nbsp;${item.TYPEOFSTUDENT}</td>
							<td>&nbsp;${item.ISRESIDENTSINCOUNTRY}</td>
							<td>&nbsp;${item.ISSPECIALTRAINING}</td>
							<td>&nbsp;${item.ISPOVERTYFAMILY}</td> --%>
							<td>&nbsp;${item.GRADE}</td>
							<%-- <td>&nbsp;${item.LENGTHOFSCHOOLING}</td> --%>
							<td>&nbsp;${item.ADMISSIONDATE}</td>
							<c:if test="${item.ISGRADUATION=='1'||item.ISGRADUATION=='是'}">
								<td>&nbsp;是</td>
							</c:if>
							<c:if test="${item.ISGRADUATION=='2'||item.ISGRADUATION=='否'}">
								<td>&nbsp;否</td>
							</c:if>
							<c:if test="${empty item.ISGRADUATION}">
								<td>&nbsp;${item.ISGRADUATION}</td>
							</c:if>
							
							<%-- <td>&nbsp;${item.ARREARAGE}</td> --%>
							<%-- <td>&nbsp;${item.STATUS}</td> --%>
