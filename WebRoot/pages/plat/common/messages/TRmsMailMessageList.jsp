...
<!--
		function  opreate(args,opCode){
			...
			if('delete' === opCode){
				...
				$.registerForm('opForm');
				baseUrl = "${ctx}/plat/common/tCommonConfig!list.action?bean.code=MAIL&menuId=${menuId}"
...
<div class="dataGrid">
<s:form action="tCommonMessage!listMailMessages" namespace="/plat/common" method="post" 
id="opForm" name="opForm">
<input type="hidden" name="menuId" value="${menuId}" id="menuId">
	 <div class="itab tabson">
		  	<ul> 
		    <li><a href="${ctx }/plat/common/tCommonMessage!listMailMessages.action?menuId=${menuId}"  class="selected" >邮件消息</a></li> 
		    <li><a href="${ctx }/plat/common/tCommonMessage!listShortMessages.action?menuId=${menuId}" >短信消息</a></li> 
		    <li><a href="${ctx }/plat/common/tCommonMessage!listSystemMessages.action?menuId=${menuId}">系统消息</a></li> 
		  	</ul>
	</div>
<div class="filter" style="padding-top: 10px;">
              ...
              <tr>
                ...
                <td>
                	<select name="message.stat" id="opresult" class="select2" style="width: 135px;">
		          	...
		          	<option value="1" ${message.stat eq '1' ? 'selected' : '' } >发送成功</option>
		          	<option value="0" ${message.stat eq '0' ? 'selected' : '' } >发送失败</option>
		          	</select>
                </td>
                 <th>创建时间：</th>
                <td>
	...
	<c:if test="${qx.add }">
		<div class="tools">
		<ul class="toolbar">
        <li class="click setting"  onclick="opreate('','config');">邮箱配置</li>
	            ...
	            <tr>
	                ...
	                <td>&nbsp;${stat.count }</td>
	                 <td>&nbsp;
                   	...
                   	<td>&nbsp;
                   		<c:choose>
                   			<c:when test="${item.STAT eq 0 }">等待发送</c:when>
                   			<c:when test="${item.STAT eq 1 }">成功</c:when>
